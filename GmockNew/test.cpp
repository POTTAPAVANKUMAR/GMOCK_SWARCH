...
public:
    MOCK_METHOD0(GetFirstNumber, int());
    MOCK_METHOD0(GetSecondNumber, int());
};

TEST(SystemUnderTestTest, SumOfTwoNumbers_WithPositiveNumbers) {
    MockCollaborator* mock_collaborator = new MockCollaborator();
    SystemUnderTest sut(mock_collaborator);

    EXPECT_CALL(*mock_collaborator, GetFirstNumber()).WillOnce(Return(2));
    EXPECT_CALL(*mock_collaborator, GetSecondNumber()).WillOnce(Return(3));

    int result = sut.SumOfTwoNumbers();

    EXPECT_EQ(5, result);
}

TEST(SystemUnderTestTest, SumOfTwoNumbers_WithNegativeNumbers) {
    MockCollaborator* mock_collaborator = new MockCollaborator();
    SystemUnderTest sut(mock_collaborator);
